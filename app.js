...
let searchQuery = "Switzerland";
let pageIndex = 1;

async function fetchData() {
  try {
    const response = await fetch(`https://api.unsplash.com/search/photos?page=${pageIndex}&per_page=30&query=${searchQuery}&client_id=pwWLMcz-JAvku4OIOrkV39WybWTEUcEqHpnmFp3w6B0`);
    ...
    if (!data.total) {
      ...
      throw new Error("Oops, rien dans la base de donnée.")
    }
    console.log(data);
